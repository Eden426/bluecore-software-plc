...
`.github/workflows/deploy.yml` has two jobs. `validate` runs on every push and every PR against `main` (`npm ci && npm run lint && npm run build` — this also exercises the prerender step via the CI/`@sparticuz` Chromium path, see above). `deploy` runs only on a push to `main` (i.e. after a PR merges), gated on `validate` passing: `vercel pull` → `vercel build --prod` → `vercel deploy --prebuilt --prod`. The build happens in the Actions runner, not in Vercel's own build container — Vercel's native build container has the same missing-shared-library problem plain `puppeteer` had, and using `--prebuilt` sidesteps it entirely regardless of which Chromium strategy is in play. The workflow needs `VERCEL_TOKEN`, `VERCEL_ORG_ID`, and `VERCEL_PROJECT_ID` configured as repo secrets (values in `.vercel/project.json` for the latter two once linked). Vercel's native GitHub integration (auto-build-on-push via Vercel's own container) is intentionally not used here. Work lands via PRs into `main`, not direct pushes — see `deploy` job's branch guard, which assumes that flow.
...
- **No router, one API route**: all navigation is same-page anchor scrolling (`html { scroll-behavior: smooth; }` in `index.css`); there's no client-side router or data-fetching layer. The one exception is `api/contact.js`, a Vercel serverless function (Node.js, not part of the Vite client build) that `Contact.jsx` POSTs to and sends the message via Resend (`resend` package). It requires a `RESEND_API_KEY` env var set on the Vercel project (not a GitHub secret — it's a runtime function env var, read at request time). The sender is Resend's shared `onboarding@resend.dev` sandbox address (no domain verified yet for `blue-core.tech`/`bluecoresoft.com`); swap `FROM_ADDRESS` in `api/contact.js` once a domain is verified in Resend. The form has a hidden honeypot field (`website`) for basic bot filtering.
